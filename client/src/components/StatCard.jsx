  ...
  return (
    <div className="glass-panel p-6 rounded-2xl relative overflow-hidden transition-all duration-300 hover:border-brand/40 group">
      ...
      <div className="flex justify-between items-start relative z-10">
        <div>
          <span className="text-xs text-dark-muted font-medium uppercase tracking-wider">{title}</span>
          <h3 className="text-3xl font-bold text-white mt-2 tracking-tight">{value}</h3>
          
          {trend && (
            <div className="flex items-center gap-1.5 mt-3">
              <span className={`text-xs font-mono font-semibold ${trendType === 'up' ? 'text-brand' : 'text-red-400'}`}>
                {trendType === 'up' ? '+' : ''}{trend}
              </span>
              <span className="text-[10px] text-dark-muted font-medium">vs last month</span>
            </div>
          )}
        </div>
